...
const flashlight = new THREE.SpotLight(0xffffff,500, 200, Math.PI / 6, 0.5, 1)
...
const player = {
    ...
    update: function() {
        ...
        if (hit.length > 0) console.log("The laser hit something at height:", hit[0].point.y);
        let height = -2.1
...
function anim() {
    ...
    console.log("Player Height:", player.position.y);
    const delta = time.getDelta();
    player.update()
    
    if (crocnpc) { 
        crocnpc.update(delta, player.position)
    }
